...
const items = [
  {
    ...
    color: 'text-zenmo-btn1',
    bg: 'bg-zenmo-btn1/[0.08]',
    title: '合规优先的执业标准',
    desc: '所有法律意见均基于个案事实，专业经验及适用法律作出判断。',
  },
  {
    icon: <MessageCircle size={22} strokeWidth={1.7} />,
    color: 'text-zenmo-btn2',
    bg: 'bg-zenmo-btn2/[0.08]',
    title: '清晰透明的沟通机制',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {items.map((item, i) => (
            <motion.div
              ...
              className="group bg-white rounded-2xl p-7 border border-gray-100 flex items-start gap-5 hover:shadow-md hover:border-gray-200 transition-all duration-300 shadow-zenmo-sm"
            >
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 transition-transform duration-300 group-hover:scale-110 ${item.bg} ${item.color}`}>
                {item.icon}
              </div>
              <div>
